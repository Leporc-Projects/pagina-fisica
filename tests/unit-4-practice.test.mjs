...
test("la familia parabólica pide rapidez |v| sin cambiar valor ni grading",()=>{const family=UNIT_4_EXERCISE_FAMILIES.find(({id})=>id==="u4-family-parabolic-energy-diagram"),values=[[0,4],[0,4],[0,5],[1,2],[2,5]],makeRandom=()=>{let index=0;return()=>{const [choice,length]=values[index++];return(choice+0.25)/length;};},es=generateLocalizedUnit4FamilyInstance(family,"es",{random:makeRandom()}),en=generateLocalizedUnit4FamilyInstance(family,"en",{random:makeRandom()}),exact=Math.sqrt(2*(es.parameters.E-es.parameters.a*es.parameters.x**2)/es.parameters.m);assert.equal(family.constraints.speedMagnitudeRequested,true);assert.match(es.prompt,/calcula \|v\|/i);assert.match(en.prompt,/calculate \|v\|/i);assert.equal(es.interaction.field.label,"|v|");assert.equal(en.interaction.field.label,"|v|");assert.ok(Math.abs(es.answer.value-exact)<=es.tolerance);assert.equal(en.answer.value,es.answer.value);assert.equal(gradeExerciseResponse(es,String(es.answer.value)).correct,true);assert.equal(gradeExerciseResponse(en,String(en.answer.value)).correct,true);});
