...
test("el registro académico declara exactamente las Unidades 1 y 2 desarrolladas", () => {
  assert.deepEqual(ACADEMIC_UNITS.map(({ number }) => number), [1, 2]);
  assert.equal(new Set(ACADEMIC_UNITS.map(({ number }) => number)).size, 2);
  assert.equal(new Set(ACADEMIC_UNITS.flatMap(({ topics }) => topics.map(({ route }) => route))).size, 14);
  assert.equal(getAcademicUnit(1)?.number, 1);
  assert.equal(getAcademicUnit(2)?.number, 2);
  assert.equal(getAcademicUnit(3), null);
  assert.equal(getAcademicUnitForContext(COURSE_IDS.PHYSICS_BASIC_1, 2)?.slug, "unidad-2");
});

test("los adapters genéricos resuelven Unidad 1 y Unidad 2 en ambos locales", () => {
  ...
  assert.deepEqual(getDevelopedAcademicUnits("en").map(({ shortTitle }) => shortTitle), ["Unit 1", "Unit 2"]);
});
...
test("renderers y catálogos consumen el registro sin imports directos de unidades", () => {
  ...
  assert.doesNotMatch(renderers, /data\/physics\/unit-[12]/);
  ...
  assert.doesNotMatch(`${unitsCatalog}\n${practiceCatalog}`, /unit\.number\s*===\s*[12]/);
});

test("el runtime de práctica materializa familias registradas de ambas unidades", () => {
  const ids = ["family-u1-vector-sum", "u2-family-second-law-acceleration"];
