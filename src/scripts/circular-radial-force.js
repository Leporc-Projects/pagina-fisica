...
import { createCircularRelationshipGeometries } from "../utils/dynamics-charts.js";
...
const createRuntime = async (root, suppliedExperience) => {
  ...
  if (!validation.valid || getSimulationModelById(canonicalExperience.modelId)?.rendererId !== "p5-circular-radial-force") throw new TypeError(validation.errors.join(" ") || "Renderer incompatible.");
  ...
  if (!(canvas instanceof HTMLElement) || !(playbackSection instanceof HTMLElement) || !(toggleButton instanceof HTMLButtonElement) || !(cutButton instanceof HTMLButtonElement) || !playbackLabel || !playbackIcon || !playbackState || !live) throw new TypeError("La interfaz circular está incompleta.");
  ...
  const resetDomain = () => {
    runtime.state = createCircularRadialState(runtime.parameters);
    runtime.readings = getCircularRadialReadings(runtime.state, runtime.parameters);
    runtime.trail = [runtime.state.position];
  };
  resetDomain();
  ...
  const syncFloating = () => floatingPlayback?.sync({ playing: runtime.playing, timeText: `${display(runtime.state.t)} s`, disabled: false });
  ...
  const updateAnalysis = () => {
    const connected = runtime.state.status === "connected";
    root.querySelector("[data-circular-connected-equations]")?.toggleAttribute("hidden", !connected);
    root.querySelector("[data-circular-free-equations]")?.toggleAttribute("hidden", connected);
    root.querySelector("[data-circular-relationship-note]")?.toggleAttribute("hidden", connected);
    const tension = root.querySelector("[data-circular-fbd-tension]");
    const string = root.querySelector("[data-circular-fbd-string]");
    const zero = root.querySelector("[data-circular-fbd-zero]");
    tension?.toggleAttribute("hidden", !connected);
    string?.toggleAttribute("hidden", !connected);
    zero?.toggleAttribute("hidden", connected);
    const note = root.querySelector("[data-circular-fbd-note]");
    if (note) note.textContent = t(locale, connected ? "circularDynamics.verticalBalance" : "circularDynamics.noHorizontalForceLong");
    setEquation("omega", runtime.readings.omega);
    setEquation("radial-acceleration", runtime.readings.radialAcceleration);
    setEquation("required-tension", runtime.readings.requiredTension);
    setEquation("required-tension-free", runtime.readings.requiredTension);
    setEquation("actual-tension", runtime.readings.tension);
    setEquation("tension-ratio", runtime.readings.requiredTension / runtime.parameters.Tmax * 100);
  };
  const updateRelationshipCharts = () => {
    const definitionsByKey = experience.parameters;
    const geometry = createCircularRelationshipGeometries(runtime.parameters, {
      vDomain: [definitionsByKey.v.minimum, definitionsByKey.v.maximum],
      RDomain: [definitionsByKey.R.minimum, definitionsByKey.R.maximum],
    });
    const speed = root.querySelector('[data-simulation-chart="circular-tension-speed"]');
    const radius = root.querySelector('[data-simulation-chart="circular-tension-radius"]');
    if (speed instanceof HTMLElement) updateSimulationLinkedChart(speed, geometry.speed, display);
    if (radius instanceof HTMLElement) updateSimulationLinkedChart(radius, geometry.radius, display);
  };
  const updateDom = () => {
    runtime.readings = getCircularRadialReadings(runtime.state, runtime.parameters);
    ...
    Object.entries(values).forEach(([key, value]) => {
      ...
      const node = root.querySelector(`[data-reading="${key}"]`);
      if (node) node.textContent = display(value);
    });
    ...
    if (badge) badge.dataset.regime = runtime.state.status;
    const ratio = runtime.readings.requiredTension / runtime.parameters.Tmax;
    const progress = root.querySelector("[data-tension-progress]");
    if (progress instanceof HTMLProgressElement) {
      progress.max = runtime.parameters.Tmax;
      progress.value = Math.min(runtime.readings.requiredTension, runtime.parameters.Tmax);
    }
    for (const [selector, value] of [["[data-meter-required]", runtime.readings.requiredTension], ["[data-meter-limit]", runtime.parameters.Tmax], ["[data-meter-ratio]", ratio * 100]]) {
      const node = root.querySelector(selector);
      if (node) node.textContent = display(value);
    }
    const note = root.querySelector("[data-meter-note]");
    if (note) note.textContent = t(locale, runtime.state.status !== "connected" ? "circularDynamics.meterDisconnected" : ratio > 1 ? "circularDynamics.meterOverload" : "circularDynamics.meterWithin");
    cutButton.disabled = runtime.state.status !== "connected";
    const flight = root.querySelector("[data-flight-status]");
    if (flight instanceof HTMLElement) flight.hidden = runtime.state.status === "connected";
    updateAnalysis();
    renderer.update();
    syncFloating();
  };
  ...
  const stepFrame = (time) => {
    ...
    const dt = Math.min(0.1, Math.max(0, (time - runtime.previous) / 1000));
    ...
    runtime.trail.push(runtime.state.position);
    if (runtime.trail.length > 240) runtime.trail.shift();
    updateDom();
  ...
  root.addEventListener("change", (event) => {
    const input = event.target;
    if (input instanceof HTMLInputElement && input.dataset.vectorToggle) {
      runtime.toggles[input.dataset.vectorToggle] = input.checked;
      renderer.update();
    }
  }, options);
  root.addEventListener("click", (event) => {
    ...
    else if (button.dataset.action === "cut" && runtime.state.status === "connected") {
      runtime.state = cutCircularString(runtime.state, runtime.parameters);
      runtime.trail.push(runtime.state.position);
      updateDom();
      announce(t(locale, "circularDynamics.cutAnnounce"));
...
export const initializeCircularRadialSimulation = async (root, experience) => {
...
export const destroyCircularRadialSimulation = async (root) => {
