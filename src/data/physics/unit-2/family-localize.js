...
import { UNIT_2_EXERCISE_FAMILIES } from "./families.js";
import { FAMILY_OBJECTIVES_EN, FAMILY_PRESENTERS_EN } from "./i18n/families.en.js";

const required = (value, context) => {
  if (value === undefined || value === null || value === "") throw new RangeError(`Missing English Unit 2 family translation: ${context}`);
  return value;
};
const parallel = (source, translated, context) => {
  if (!Array.isArray(translated) || source.length !== translated.length) throw new RangeError(`English Unit 2 family translation changed structure: ${context}`);
  return translated;
};

export const localizeUnit2ExerciseFamily = (family, locale) => {
...
export const getLocalizedUnit2ExerciseFamilies = (locale) => UNIT_2_EXERCISE_FAMILIES.map((family) => localizeUnit2ExerciseFamily(family, locale));
...
const localizeInstance = (instance, locale) => {
  ...
  const family = localizeUnit2ExerciseFamily(UNIT_2_EXERCISE_FAMILIES.find((candidate) => candidate.id === instance.familyId), locale);
  ...
  const localizedFields = fields.map((field, index) => ({ ...field, label: required(labels[index], `${instance.familyId}.fields.${field.id}`) }));
  const solution = parallel(instance.solution, translated.solution, `${instance.familyId}.solution`);
  return {
    ...instance, objectives: family.objectives, title: translated.title, prompt: translated.prompt,
    ...
    interaction: instance.interaction.kind === "number" ? { ...instance.interaction, field: localizedFields[0] } : { ...instance.interaction, fields: localizedFields },
    feedback: family.feedback,
  };
};

export const generateLocalizedUnit2FamilyInstance = (family, locale, options = {}) => {
  assertSupportedLocale(locale);
