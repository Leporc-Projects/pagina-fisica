...
import { UNIT_2_EXERCISES } from "./exercises.js";
import exercisesEn from "./i18n/exercises.en.js";
import { getLocalizedUnit2ExerciseFamilies } from "./family-localize.js";

const required = (value, context) => {
  if (value === undefined || value === null || value === "") throw new RangeError(`Missing English Unit 2 exercise translation: ${context}`);
  return value;
};
const parallel = (source, translated, context) => {
  if (!Array.isArray(translated) || source.length !== translated.length) throw new RangeError(`English Unit 2 exercise translation changed structure: ${context}`);
  return translated;
};
...
export const localizeUnit2Exercise = (exercise, locale) => {
  assertSupportedLocale(locale);
  if (!exercise || locale === "es") return exercise;
  const translated = required(exercisesEn[exercise.id], exercise.id);
  const solution = parallel(exercise.solution, translated.solution, `${exercise.id}.solution`);
  return {
    ...exercise,
    title: translated.title, prompt: translated.prompt,
    ...
    solution: exercise.solution.map((step, index) => ({ ...step, title: solution[index].title, text: solution[index].text })),
...
export const getLocalizedUnit2Exercises = (locale) => UNIT_2_EXERCISES.map((exercise) => localizeUnit2Exercise(exercise, locale));
export const getLocalizedUnit2BankItems = (locale) => [...getLocalizedUnit2Exercises(locale), ...getLocalizedUnit2ExerciseFamilies(locale)];
export { generateLocalizedUnit2FamilyInstance } from "./family-localize.js";
