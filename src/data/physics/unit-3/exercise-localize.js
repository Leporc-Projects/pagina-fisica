...
export const localizeUnit3Exercise = (exercise, locale) => {
  ...
  return { ...exercise, title: translated.title, prompt: translated.prompt, objectives: parallel(exercise.objectives, translated.objectives, `${exercise.id}.objectives`), prerequisites: parallel(exercise.prerequisites, translated.prerequisites, `${exercise.id}.prerequisites`), hints: parallel(exercise.hints, translated.hints, `${exercise.id}.hints`), solution: exercise.solution.map((step, index) => ({ ...step, title: translatedSolution[index].title, text: translatedSolution[index].text })), answer: { ...exercise.answer, ...(exercise.answer.kind === "text" ? { presentation: translated.answerDisplay } : translated.answerDisplay ? { display: translated.answerDisplay } : {}) }, interaction: localizeInteraction(exercise.interaction, translated, exercise.id), feedback: { ...exercise.feedback, correct: t(locale, "exercise.feedback.correct"), incorrect: t(locale, "exercise.feedback.incorrect") } };
};
