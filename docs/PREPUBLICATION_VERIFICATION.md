...
The four pulley drawings now share explicit mechanical vocabulary: grooved wheels, fixed axles, table bracket, ceiling supports, rope anchors, mass eyelets, movable yokes and continuous hangers. Pure geometry exposes every endpoint and the model owns scenario-specific terminal surfaces in metres. Contact is selected by the earliest positive analytic crossing, independent of viewport; playback preserves the immediately pre-impact velocity and acceleration and the history chart marks the same event. Static spokes replace the former arbitrary time/position rotation.
